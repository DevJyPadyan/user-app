...
// Function to fetch and display all weeks' data for a specific hostel
const fetchAllWeeksData = () => {
    const dbRef = ref(db, `Hostel details/${hostelName}/weeks`);
    ...
    onValue(dbRef, (snapshot) => {
        ...
        snapshot.forEach(weekSnapshot => {
            const weekNumber = weekSnapshot.key;
...
// Function to create a new section for each week
const createWeekSection = (weekNumber) => {
    ...
    title.innerText = `Week ${weekNumber}`;
...
// Function to fetch and append meal details for a specific week
...
window.addEventListener('load', fetchAllWeeksData);*/
...
const createWeekSection = (weekNumber) => {
    ...
    const title = document.createElement('h3');
    title.innerText = `Week 1 details:`; // Explicitly setting the title
    sectionWrapper.appendChild(title);
...
window.addEventListener('load', fetchWeek1Data);
